...
public class VoteService {
    ...
    public void saveVote(Long roomId, User user, VoteRequestDto requestDto) {
        ...
        if (requestDto.getTopic()) { // topicA를 골랐을 때
            pcikSave(true, chatRoom, requestDto, vote, user);
            vote.winnerA(requestDto);
            voteRepository.save(vote);
        } else {//topicB를 골랐을 때
            pcikSave(false, chatRoom, requestDto, vote, user);
            vote.winnerB(requestDto);
            voteRepository.save(vote);
        }
    }

    private void pcikSave(boolean pick, ChatRoom chatRoom, VoteRequestDto requestDto, Vote vote, User user) {
        SaveVote saveVote = new SaveVote(pick, chatRoom, requestDto.getPoint(), vote, user); // savevote 추가
    ...
    public Vote winVote(Long chatRoomId) {
        ...
        if (vote.getTopicACnt() > vote.getTopicBCnt()) { // topicA가 이겻을 경우
            ...
            saveVoteList(chatRoomId, winRate, saveVotesTrueList, userWinnerList, "투표 승리");
            ...
            saveVoteList(chatRoomId, winRate, saveVotesFalseList, userWinnerList, "투표 승리");
            ...
            saveVoteList(chatRoomId, 1f, saveVotesTieList, userWinnerList, "투표 승리");
        } else {//투표가 동률일경우
            List<SaveVote> saveVotesTieList = saveVoteRepository.findAllByChatRoom_Id(chatRoomId);

            saveVoteList(chatRoomId, 1f, saveVotesTieList, userWinnerList, "투표 무승부" );
        }
        return vote;
    }

    private void saveVoteList(Long roomId, Float winRate, List<SaveVote> saveVotesTrueList, List<User> userWinnerList, String content) {
        ...
        for (User user : userWinnerList) {
            ...
            userRepository.save(user);
            Point point = new Point(content, (long) (saveVote.getPoint() * winRate),  user); //포인트 내역 생성
